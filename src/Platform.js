class Platform {
  ...
  addSkeleton() {
    this.skeleton = this.scene.physics.add
      .sprite(0, 0, "gameAtlas", "floor.png")
      .setVisible(false)
      .setOrigin(0, 0)
  ...
  addTexture() {
    this.tileTexture = new TileSprite(this.scene, 0, 0, this.config.w, this.config.h, this.config.theme.texture)
      .setOrigin(0, 0);
    this.tileTexture.tilePositionX -= this.config.w / 2 + 40;
  }
  addTopGround() {
    this.topGround = new TileSprite(this.scene, 0, 0, this.config.w, 49, "platformFloor")
      .setOrigin(0, 0)
